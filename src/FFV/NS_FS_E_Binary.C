...
void FFV::NS_FS_E_CBC()
{
  int d_length;          /// 一次元配列長
  
  // local variables
  REAL_TYPE dt = deltaT;               /// 時間積分幅
  double flop_count;                   /// 浮動小数演算数
  ...
  // d_wk2 Poissonのソース項1　反復毎に変化するソース項，摩擦速度，発散値, div(u)の値を保持，出力のところまでは再利用しないこと
  // d_bcd IDのビットフラグ
  // d_bcp 圧力のビットフラグ
  // d_bcv 速度のビットフラグ
  ...
  fb_copy_real_(d_p0, d_p, &d_length);
  
  d_length *= 3;
  fb_copy_real_(d_v0, d_v, &d_length);
  ...
  switch (C.AlgorithmF) {
    ...
    case Control::Flow_FS_AB2:
      ...
      flop_count = 0.0;
      BC.mod_Pvec_Flux(vc, v0, bcv, CurrentTime, &C, v_mode, v00, flop_count);
    ...
    case Control::Flow_FS_AB_CN:
      ...
      TIMING_start(tm_pvec_flux);
      flop_count = 0.0;
      BC.mod_Pvec_Flux(wv, v0, bcv, CurrentTime, &C, v_mode, v00, flop_count);
      TIMING_stop(tm_pvec_flux, flop_count);
      break;
  ...
  switch (C.AlgorithmF) {
    ...
    case Control::Flow_FS_AB_CN:
      ...
      flop_count = 0.0;
      BC.mod_Vis_EE(vc, v0, half, bcv, CurrentTime, dt, v00, flop_count);
  ...
  flop_count = 0.0;
  BC.OuterVBC_Pseudo(vc, v0, bcv, CurrentTime, dt, &C, v00, flop_count);
  ...
  flop_count = 0.0;
  BC.mod_Psrc_VBC(src0, vc, v0, coef, bcv, CurrentTime, dt, &C, v00, flop_count);
  ...
  for (ICp->LoopCount=0; ICp->LoopCount< ICp->get_ItrMax(); ICp->LoopCount++) {
    ...
    flop_count=0.0;
    BC.mod_div(src1, bcv, coef, CurrentTime, v00, m_buf, flop_count);
  ...
  flop_count=0.0;
  BC.InnerVBC(v, bcv, CurrentTime, v00, flop_count);
  BC.OuterVBC(v, vc, bcv, CurrentTime, dt, &C, v00, flop_count);
