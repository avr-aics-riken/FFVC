  ...
  for (int n=1; n<=NoBC; n++) {
    ...
    switch (typ) {
      case SPEC_VEL_WH:
        ...
        cbc_hbc_drchlt_(d_t, size, gc, st, ed, d_bh1, &n, &tc);
        break;
    }
  }
  /*
   ...
   cbc_vbc_drchlt_cc_(v, size, gc, st, ed, v00, bv, &n, vec);
   }
   }*/
}
...
void SetBC3D::assign_Velocity(REAL_TYPE* d_v, int* d_bv, REAL_TYPE tm, REAL_TYPE* v00, bool clear)
{
  ...
  for (int n=1; n<=NoBC; n++) {
    ...
    switch (typ) {
      ...
      case SPEC_VEL_WH:
        ...
        cbc_vibc_drchlt_(d_v, size, gc, st, ed, v00, d_bv, &n, vec);
        break;
    }
  }
  ...
  for (int face=0; face<NOFACE; face++) {
    ...
    if ( typ == OBC_SPEC_VEL ) {
      ...
      cbc_vobc_drchlt_(d_v, size, gc, v00, d_bv, &face, vec);
    }
  }
}
  ...
  if ( isCDS ) { // Cut-Distance
    ...
    for (int n=1; n<=NoBC; n++) {
      ...
      switch ( cmp[n].getType() ) {
        ...
        case OUTFLOW:
          cbc_vibc_outflow_(d_v, size, gc, st, ed, d_bv, &n);
          break;
...
void SetBC3D::mod_div(REAL_TYPE* d_div, int* d_bv, REAL_TYPE coef, REAL_TYPE tm, REAL_TYPE* v00, REAL_TYPE* avr, double& flop)
{
  ...
  if ( isCDS ) { // Cut-Distance
    for (int n=1; n<=NoBC; n++) {
      ...
      switch (typ) {
        case OUTFLOW:
          //cbc_div_ibc_oflow_vec_(div, size, gc, st, ed, v00, &coef, bv, &n, &avr[2*n], &flop);
          break;
          
        case SPEC_VEL:
        case SPEC_VEL_WH:
          cmp[n].val[var_Velocity] = extractVel_IBC(n, vec, tm, v00, flop); // 指定された無次元平均流速
          cbc_div_ibc_drchlt_(d_div, size, gc, st, ed, v00, &coef, d_bv, &n, vec, &flop);
          break;
    ...
    for (int n=1; n<=NoBC; n++) {
      ...
      switch (typ) {
        case OUTFLOW:
          cbc_div_ibc_oflow_vec_(d_div, size, gc, st, ed, v00, &coef, d_bv, &n, &avr[2*n], &flop);
          break;
  ...
  for (int face=0; face<NOFACE; face++) {
    ...
    switch (typ) {
      case OBC_OUTFLOW:
        cbc_div_obc_oflow_vec_(d_div, size, gc, &face, v00, &coef, d_bv, vec, &flop); // vecは流用
      ...
      case OBC_WALL:
        ...
        cbc_div_obc_drchlt_(d_div, size, gc, &face, v00, &coef, d_bv, vec, &flop);
...
void SetBC3D::mod_Dir_Forcing(REAL_TYPE* d_v, int* d_bd, float* d_cvf, REAL_TYPE* v00, double &flop)
{
  ...
  for (int n=1; n<=NoBC; n++) {
    if ( cmp[n].isFORCING() ) {
      ...
      switch ( cmp[n].getType() ) {
        case HEX:
          if ( cmp[n].get_sw_HexDir() ) cbc_hex_dir_ (d_v, size, gc, st, ed, d_bd, d_cvf, &n, v00, vec, &flop);
          break;
...
void SetBC3D::mod_Pvec_Forcing(REAL_TYPE* d_vc, REAL_TYPE* d_v, int* d_bd, float* d_cvf, REAL_TYPE* v00, REAL_TYPE dt, double &flop)
{
  ...
  for (int n=1; n<=NoBC; n++) {
    ...
    switch ( cmp[n].getType() ) {
      case HEX:
        cbc_hex_force_pvec_(d_vc, size, gc, st, ed, d_bd, d_cvf, d_v, &n, v00, &dt, vec, &cmp[n].ca[0], &flop);
        break;
...
void SetBC3D::mod_Psrc_Forcing(REAL_TYPE* d_src, REAL_TYPE* d_v, int* d_bd, float* d_cvf, REAL_TYPE dh, REAL_TYPE* v00, REAL_TYPE** c_array, double &flop)
{
  ...
  for (int n=1; n<=NoBC; n++) {
    ...
    if ( cmp[n].isFORCING() ) cbc_force_keep_vec_(w_ptr, csz, st, ed, d_v, size, gc);

    switch ( cmp[n].getType() ) {
      case HEX:
        cbc_hex_psrc_(d_src, size, gc, st, ed, d_bd, d_cvf, w_ptr, csz, &n, v00, &dh, vec, &cmp[n].ca[0], &flop);
        break;
...
void SetBC3D::mod_Vdiv_Forcing(REAL_TYPE* d_v, int* d_bd, float* d_cvf, REAL_TYPE* d_div, REAL_TYPE dt, REAL_TYPE dh, REAL_TYPE* v00, REAL_TYPE* am, REAL_TYPE** c_array, double &flop)
{
  ...
  for (int n=1; n<=NoBC; n++) {
    if ( cmp[n].isFORCING() ) {
      ...
      switch ( cmp[n].getType() ) {
        case HEX:
          cbc_hex_force_vec_(d_v, d_div, size, gc, st, ed, d_bd, d_cvf, w_ptr, csz, &n, v00, &dt, &dh, vec, &cmp[n].ca[0], &am[2*n], &flop);
          break;
...
void SetBC3D::mod_Pvec_Flux(REAL_TYPE* d_wv, REAL_TYPE* d_v, int* d_bv, REAL_TYPE tm, Control* C, int v_mode, REAL_TYPE* v00, double& flop)
{
    ...
    for (int n=1; n<=NoBC; n++) {
      ...
      if( cmp[n].isEns() ) {
        ...
        if ( (typ==SPEC_VEL) || (typ==SPEC_VEL_WH) ) {
          extractVel_IBC(n, vec, tm, v00, flop);
          cbc_pvec_vibc_specv_(d_wv, size, gc, st, ed, &dh, v00, &rei, d_v, d_bv, &n, vec, &flop);
        }
        else if ( typ==OUTFLOW ) {
        }      
      }
    ...
    for (int n=1; n<=NoBC; n++) {
      ...
      if( cmp[n].isEns() ) {
        ...
        if ( (typ==SPEC_VEL) || (typ==SPEC_VEL_WH) ) {
          ...
          cbc_pvec_vibc_oflow_(d_wv, size, gc, st, ed, &dh, &rei, d_v, d_bv, &n, vec, &flop);
        }      
      }
    }
  }
  ...
  for (int face=0; face<NOFACE; face++) {
    ...
    switch ( typ ) {
      case OBC_SPEC_VEL:
        extractVel_OBC(face, vec, tm, v00, flop);
        cbc_pvec_vobc_specv_(d_wv, size, gc, &dh, v00, &rei, d_v, d_bv, vec, &face, &flop);
        break;
        
      case OBC_WALL:
        extractVel_OBC(face, vec, tm, v00, flop);
        cbc_pvec_vobc_wall_(d_wv, size, gc, &dh, v00, &rei, d_v, d_bv, vec, &face, &flop);
        break;
        
      case OBC_SYMMETRIC:
        cbc_pvec_vobc_symtrc_(d_wv, size, gc, &dh, &rei, d_v, d_bv, &face, &flop);
        break;
        
      case OBC_OUTFLOW:
        vec[0] = vec[1] = vec[2] = C->V_Dface[face];
        cbc_pvec_vobc_oflow_(d_wv, size, gc, &dh, v00, &rei, d_v, d_bv, vec, &face, &flop);
        break;
    }
  }
  ...
  for (int face=0; face<NOFACE; face++) {
    ...
    switch ( typ ) {
      ...
      case OBC_SYMMETRIC:
        ...
        //vec[0] = vec[1] = vec[2] = 0.0;
        //cbc_div_obc_drchlt_(div, size, gc, &face, v00, &coef, d_bv, vec, &flop);
        break;
        
      case OBC_OUTFLOW:
        vel = C->V_Dface[face] * dt / dh;
        cbc_div_obc_oflow_pvec_(d_div, size, gc, &face, v00, &vel, &coef, d_bv, d_v0, &flop);
        break;
...
void SetBC3D::mod_Vis_EE(REAL_TYPE* d_vc, REAL_TYPE* d_v0, REAL_TYPE cf, int* d_bx, REAL_TYPE tm, REAL_TYPE dt, REAL_TYPE* v00, double& flop)
{
  ...
  for (int n=1; n<=NoBC; n++) {
    ...
    cmp[n].getBbox(st, ed);
    cbc_vis_ee_vbc_(d_vc, size, gc, st, ed, &dh, &dt, v00, &rei, d_v0, d_bx, &n, &cf, vec, &flop);
  }
}
  ...
  for (int face=0; face<NOFACE; face++) {
    ...
    if ( F == OBC_PERIODIC ) {
      ...
      switch ( F ) {
        ...
        case OBC_TRC_FREE:
          cbc_pobc_drchlt_ (d_p, size, gc, &face, &pv);
          break;
          
        case OBC_FAR_FIELD:
          cbc_pobc_neumann_(d_p, size, gc, &face);
          break;
      }
    }
  }
...
void SetBC3D::OuterVBC(REAL_TYPE* d_v, REAL_TYPE* d_vc, int* d_bv, REAL_TYPE tm, REAL_TYPE dt, Control* C, REAL_TYPE* v00, double& flop)
{
  ...
  for (int face=0; face<NOFACE; face++) {
    ...
    switch ( obc[face].get_Class() ) {
      case OBC_OUTFLOW:
        cbc_vobc_update_(d_v, size, gc, d_vc, &face);
        break;
      ...
      case OBC_TRC_FREE:
        cbc_vobc_tfree_(d_v, size, gc, &face, &flop);
        break;
        
      case OBC_FAR_FIELD:
        cbc_vobc_neumann_(d_v, size, gc, &face);
        break;
  ...
  for (int face=0; face<NOFACE; face++) {
    ...
    switch ( obc[face].get_Class() ) {
      case OBC_OUTFLOW:
        ...
        cbc_vobc_outflow_(d_vc, size, gc, &v_cnv, d_bv, &face, d_v0, &flop);
        break;
    }
...
void SetBC3D::mod_Vis_CN(REAL_TYPE* vc, REAL_TYPE* wv, REAL_TYPE cf, int* bx, REAL_TYPE* wk, REAL_TYPE tm, REAL_TYPE dt, REAL_TYPE omg, Control* C, REAL_TYPE* res, int LS, REAL_TYPE* v00, double& flop)
{
  ...
  for (int n=1; n<=NoBC; n++) {
    if ( cmp[n].isVBC() ) {
      ...
      switch (LS) {
        case JACOBI:
          cbc_vis_cn_mod_jcb_(vc, size, gc, st, ed, &dh, &dt, v00, &rei, &omg, wv, bx, wk, &cf, res, vec, flop);
          break;
          
        case SOR:
          cbc_vis_cn_mod_sor_(vc, size, gc, st, ed, &dh, &dt, v00, &rei, &omg, wv, bx, &cf, res, vec, flop);
          break;
        ...
        if ( nID[face] < 0 ) {
          for (int k=1; k<=kx; k++) {
            for (int j=1; j<=jx; j++) {
              for (int i=1-gd; i<=0; i++) {
                for (int l=0; l<3; l++) {
                  m0 = FBUtility::getFindexV3DEx(sz, gd, l, i   , j, k);
                  m1 = FBUtility::getFindexV3DEx(sz, gd, l, ix+i, j, k);
                  d_v[m0] = d_v[m1];
                }
              }
            }
          ...
          for (int k=1; k<=kx; k++) {
            for (int j=1-gd; j<=0; j++) {
              for (int i=1; i<=ix; i++) {
                for (int l=0; l<3; l++) {
                  m0 = FBUtility::getFindexV3DEx(sz, gd, l, i, j   , k);
                  m1 = FBUtility::getFindexV3DEx(sz, gd, l, i, jx+j, k);
                  d_v[m0] = d_v[m1];
                }
              }
            }
          ...
          for (int k=1-gd; k<=0; k++) {
            for (int j=1; j<=jx; j++) {
              for (int i=1; i<=ix; i++) {
                for (int l=0; l<3; l++) {
                  m0 = FBUtility::getFindexV3DEx(sz, gd, l, i, j, k   );
                  m1 = FBUtility::getFindexV3DEx(sz, gd, l, i, j, kx+k);
                  d_v[m0] = d_v[m1];
                }
              }
            }
          ...
          for (int k=1; k<=kx; k++) {
            for (int j=1; j<=jx; j++) {
              for (int i=ix+1; i<=ix+gd; i++) {
                for (int l=0; l<3; l++) {
                  m0 = FBUtility::getFindexV3DEx(sz, gd, l, i   , j, k);
                  m1 = FBUtility::getFindexV3DEx(sz, gd, l, i-ix, j, k);
                  d_v[m0] = d_v[m1];
                }
              }
            }
          ...
          for (int k=1; k<=kx; k++) {
            for (int j=jx+1; j<=jx+gd; j++) {
              for (int i=1; i<=ix; i++) {
                for (int l=0; l<3; l++) {
                  m0 = FBUtility::getFindexV3DEx(sz, gd, l, i, j   , k);
                  m1 = FBUtility::getFindexV3DEx(sz, gd, l, i, j-jx, k);
                  d_v[m0] = d_v[m1];
                }
              }
            }
          ...
          for (int k=kx+1; k<=kx+gd; k++) {
            for (int j=1; j<=jx; j++) {
              for (int i=1; i<=ix; i++) {
                for (int l=0; l<3; l++) {
                  m0 = FBUtility::getFindexV3DEx(sz, gd, l, i, j, k   );
                  m1 = FBUtility::getFindexV3DEx(sz, gd, l, i, j, k-kx);
                  d_v[m0] = d_v[m1];
                }
              }
            }
            ...
            for (int l=0; l<3; l++) {
              ...
              if ( DECODE_CMP(d_bx[m1]) == odr ) {
                for (ii=1-gd; ii<=0; ii++) {
                  ...
                  m2 = FBUtility::getFindexV3DEx(sz, gd, l, i+ii, j, k);
                  d_v[m0] = d_v[m2];
                }
              }
            }
            ...
            for (int l=0; l<3; l++) {
              ...
              if ( DECODE_CMP(d_bx[m1]) == odr ) {
                for (jj=1-gd; jj<=0; jj++) {
                  ...
                  m2 = FBUtility::getFindexV3DEx(sz, gd, l, i, j+jj, k);
                  d_v[m0] = d_v[m2];
                }
              }              
            }
            ...
            for (int l=0; l<3; l++) {
              ...
              if ( DECODE_CMP(d_bx[m1]) == odr ) {
                for (kk=1-gd; kk<=0; kk++) {
                  ...
                  m2 = FBUtility::getFindexV3DEx(sz, gd, l, i, j, k+kk);
                  d_v[m0] = d_v[m2];
                }
              }         
            }
