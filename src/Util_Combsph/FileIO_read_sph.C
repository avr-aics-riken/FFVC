 ...
 * @return      true=success, false=fault
 */
bool FileIO_SPH::ReadFile(const char* fname,
                          DataType* data_type,
                          unsigned int voxsize[3],
                          float voxorg[3],
                          float voxpit[3],
...
{
  if( !fname || !voxsize || !voxorg || !voxpit
             || !data_type || !step || !time ) return false;
 ...
 * @return      true=success, false=fault
 */
bool FileIO_SPH::GetDataType(const char* fname,
                            int* real_type,
                            int* data_type)
{
  if( !fname || !real_type || !data_type ) return false;
