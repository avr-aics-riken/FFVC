...
!! @param [in]  cut_l    カット情報 dir=0,1,2
!! @param [in]  cut_u    カット情報 dir=3,4,5
!! @param [out] flop     浮動小数点演算数
!<
subroutine pvec_muscl_cds (wv, sz, g, dh, c_scheme, rei, v, vf, bv, bp, bcd, v_mode, cut_l, cut_u, flop)
implicit none
...
integer, dimension(1-g:sz(1)+g, 1-g:sz(2)+g, 1-g:sz(3)+g)   ::  cut_l, cut_u
...
do i=1,ix
  ...
  ! カット情報
  dww= cut_l(i-1,j  ,k  ) ! d_{i-1}^-
  dw = cut_l(i  ,j  ,k  ) ! d_{i}^-
  de = cut_l(i  ,j  ,k  ) ! d_{i}^+
  dee= cut_l(i+1,j  ,k  ) ! d_{i+1}^+
  
  dss= cut_l(i  ,j-1,k  ) ! d_{j-1}^-
  ds = cut_l(i  ,j  ,k  ) ! d_{j}^-
  dn = cut_u(i  ,j  ,k  ) ! d_{j}^+
  dnn= cut_u(i  ,j+1,k  ) ! d_{j+1}^+

  dbb= cut_u(i  ,j  ,k-1) ! d_{k-1}^-
  db = cut_u(i  ,j  ,k  ) ! d_{k}^-
  dt = cut_u(i  ,j  ,k  ) ! d_{k}^+
  dtt= cut_u(i  ,j  ,k+1) ! d_{k+1}^+
...
!<
subroutine update_vec_cds (v, div, sz, g, delta_t, dh, vc, p, bp, bv, bcd, cut_l, cut_u, flop)
implicit none
include '../FB/ffv_f_params.h'
...
!! @param[out] flop flop count
!<
subroutine divergence_cds (div, sz, g, coef, v, bv, bcd, cut_l, cut_u, flop)
implicit none
include '../FB/ffv_f_params.h'
...
!!    - 境界条件は必要か？
!<
subroutine eddy_viscosity_cds (vt, sz, g, dh, re, cs, v, cut_l, cut_u, vt_range, yp_range, flop)
implicit none
include '../FB/ffv_f_params.h'
...
real, dimension(1-g:sz(1)+g, 1-g:sz(2)+g, 1-g:sz(3)+g, 3)   ::  v
integer, dimension(1-g:sz(1)+1, 1-g:sz(2)+1, 1-g:sz(3)+1)   ::  cut_l, cut_u
...
!     ------- Y+ & fs--
      fs =1.0
      AAA= cut_l(i,j,k) * cut_l(i,j,k)   &
          *cut_l(i,j,k) * cut_u(i,j,k)   &
          *cut_u(i,j,k) * cut_u(i,j,k)
      u1 = v(i,j,k,1)
      u2 = v(i,j,k,2)
      u3 = v(i,j,k,3)
      ...
      IF ( (AAA < 1.0) .AND. (Vmag >= 0.001) ) THEN
        !DIS = AMIN1(cut_l(i,j,k), cut_l(i,j,k),    &
        !            cut_l(i,j,k), cut_u(i,j,k),    &
        !            cut_u(i,j,k), cut_u(i,j,k) ) * dh
        Tw = Vmag/(DIS*RE)
        Ut0= SQRT(Tw)
        YP = DIS*Ut0*RE
        Up1= Vmag
