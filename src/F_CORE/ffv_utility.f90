...
!! @param [out]      flop   flop count
!<
subroutine calc_production_rate (P_ave, sz, dh, g, v_ave, R, bid, nadd, flop)
implicit none
include 'ffv_f_params.h'
integer                                                   ::  i, j, k, ix, jx, kx, g, idx
integer, dimension(3)                                     ::  sz
...
flop = flop + dble(ix)*dble(jx)*dble(kx)*121.0d0 + 36.0d0
...
b_b1 = 1.0
b_t1 = 1.0
if ( ibits(idx, bc_face_W, bitw_5) /= 0 ) b_w1 = 0.0
if ( ibits(idx, bc_face_E, bitw_5) /= 0 ) b_e1 = 0.0
if ( ibits(idx, bc_face_S, bitw_5) /= 0 ) b_s1 = 0.0
if ( ibits(idx, bc_face_N, bitw_5) /= 0 ) b_n1 = 0.0
if ( ibits(idx, bc_face_B, bitw_5) /= 0 ) b_b1 = 0.0
if ( ibits(idx, bc_face_T, bitw_5) /= 0 ) b_t1 = 0.0


! セルセンターからの壁面修正速度 > 3 flop
uq = -Up0
vq = -Vp0
wq = -Wp0
...
!! @param [out]  flop   flop count
!<
subroutine calc_dissipation_rate (E_ave, sz, dh, g, nu, v, v_ave, bid, nadd, flop)
implicit none
include 'ffv_f_params.h'
integer                                                   ::  i, j, k, ix, jx, kx, g, idx
integer, dimension(3)                                     ::  sz
...
flop = flop + dble(ix)*dble(jx)*dble(kx)*107.0d0 + 36.0d0


!$OMP PARALLEL &
!$OMP PRIVATE(idx, actv, uq, vq, wq, tmp) &
!$OMP PRIVATE(Up0, Ue1, Uw1, Us1, Un1, Ub1, Ut1) &
!$OMP PRIVATE(Vp0, Ve1, Vw1, Vs1, Vn1, Vb1, Vt1) &
!$OMP PRIVATE(Wp0, We1, Ww1, Ws1, Wn1, Wb1, Wt1) &
!$OMP PRIVATE(b_e1, b_w1, b_n1, b_s1, b_t1, b_b1) &
...
!$OMP FIRSTPRIVATE(ix, jx, kx, rx, ry, rz, nu, val1, val2)
!$OMP DO SCHEDULE(static)
...
! 散逸項
! 36 flop
E11 = tmp * ( gradUt11*gradU11 + gradUt12*gradU21 + gradUt13*gradU31 )
E12 = tmp * ( gradUt11*gradU12 + gradUt12*gradU22 + gradUt13*gradU32 )
E13 = tmp * ( gradUt11*gradU13 + gradUt12*gradU23 + gradUt13*gradU33 )
E22 = tmp * ( gradUt21*gradU12 + gradUt22*gradU22 + gradUt23*gradU32 )
E23 = tmp * ( gradUt21*gradU13 + gradUt22*gradU23 + gradUt23*gradU33 )
E33 = tmp * ( gradUt31*gradU13 + gradUt32*gradU23 + gradUt33*gradU33 )
...
!! @param [out]      flop   flop count
!<
subroutine calc_turb_transport_rate (T_ave, sz, dh, g, v, v_ave, R, bid, nadd, flop)
implicit none
include 'ffv_f_params.h'
integer                                                   ::  i, j, k, ix, jx, kx, g, idx
real, dimension(3)                                        ::  dh
...
flop = flop + dble(ix)*dble(jx)*dble(kx)*84.0d0 + 36.0d0

!$OMP PARALLEL &
!$OMP PRIVATE(idx, actv) &
!$OMP PRIVATE(v1p, v2p, v3p, t1, t2, t3) &
...
T11 = - ( t1 * (R11_e1 - R11_w1) &
        + t2 * (R11_n1 - R11_s1) &
        + t3 * (R11_t1 - R11_b1) &
        ) * actv

T12 = - ( t1 * (R12_e1 - R12_w1) &
        + t2 * (R12_n1 - R12_s1) &
        + t3 * (R12_t1 - R12_b1) &
        ) * actv

T13 = - ( t1 * (R13_e1 - R13_w1) &
        + t2 * (R13_n1 - R13_s1) &
        + t3 * (R13_t1 - R13_b1) &
        ) * actv

T22 = - ( t1 * (R22_e1 - R22_w1) &
        + t2 * (R22_n1 - R22_s1) &
        + t3 * (R22_t1 - R22_b1) &
        ) * actv

T23 = - ( t1 * (R23_e1 - R23_w1) &
        + t2 * (R23_n1 - R23_s1) &
        + t3 * (R23_t1 - R23_b1) &
        ) * actv

T33 = - ( t1 * (R33_e1 - R33_w1) &
        + t2 * (R33_n1 - R33_s1) &
        + t3 * (R33_t1 - R33_b1) &
        ) * actv
...
flop = flop + dble(ix)*dble(jx)*dble(kx)*89.0d0 + 33.0d0
...
if ( ibits(idx, bc_n_W, 1) == 0 ) pw1 = p0
if ( ibits(idx, bc_n_E, 1) == 0 ) pe1 = p0
if ( ibits(idx, bc_n_S, 1) == 0 ) ps1 = p0
if ( ibits(idx, bc_n_N, 1) == 0 ) pn1 = p0
if ( ibits(idx, bc_n_B, 1) == 0 ) pb1 = p0
if ( ibits(idx, bc_n_T, 1) == 0 ) pt1 = p0

! 壁面の場合の参照速度の修正 (Dirichlet 条件)
if ( ibits(idx, bc_d_W, 1) == 0 ) pw1 = -p0
if ( ibits(idx, bc_d_E, 1) == 0 ) pe1 = -p0
if ( ibits(idx, bc_d_S, 1) == 0 ) ps1 = -p0
if ( ibits(idx, bc_d_N, 1) == 0 ) pn1 = -p0
if ( ibits(idx, bc_d_B, 1) == 0 ) pb1 = -p0
if ( ibits(idx, bc_d_T, 1) == 0 ) pt1 = -p0
