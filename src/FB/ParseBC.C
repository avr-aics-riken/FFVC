...
//@brief iTableのポインタを返す
IDtable* ParseBC::get_IDtable_Ptr(void)
{
  return iTable;
}
...
void ParseBC::printCompo(FILE* fp, REAL_TYPE* nv, int* gci, MaterialList* mat)
{
  ...
  if ( isComponent(SPEC_VEL) || isComponent(SPEC_VEL_WH) ) {
    fprintf(fp, "\n\t[SPECIFIED_VELOCITY]\n");
    fprintf(fp, "\t no                    Label    ID   def    i_st    i_ed    j_st    j_ed    k_st    k_ed   Area[m*m]  ( Computed normal from ID [x,y,z])\n");
    
    for(n=1; n<=NoBC; n++){
      if ( (compo[n].getType() == SPEC_VEL) || (compo[n].getType() == SPEC_VEL_WH) )  {
        fprintf(fp,"\t%3d %24s %5d %5d %7d %7d %7d %7d %7d %7d %11.4e  (% 10.3e % 10.3e % 10.3e)\n",
                ...
                compo[n].area, nv[3*n+0], nv[3*n+1], nv[3*n+2]); // nv[]はモデルから計算した法線
      }
    }
    fprintf(fp, "\n");
    fprintf(fp, "\t no                    Label    ID   normal_x   normal_y   normal_z  Direction     Q[m^3/s]   Amp.[m/s]   Freq.[Hz]  Phase[rad] Intcpt[m/s] Strauhal[-]\n");
    ...
    for(n=1; n<=NoBC; n++){
      if ( (compo[n].getType() == SPEC_VEL) || (compo[n].getType() == SPEC_VEL_WH) )  {
        ...
        if ( compo[n].get_sw_V_profile() == CompoList::vel_zero ) {
          ...
          fprintf(fp,"%12.4e ",( compo[n].get_sw_V_profile() == CompoList::vel_constant ) ? compo[n].ca[CompoList::bias]*compo[n].area : compo[n].ca[CompoList::amplitude]*compo[n].area );
        }
        fprintf(fp,"%11.3e %11.3e %11.3e %11.3e %11.4e\n",
