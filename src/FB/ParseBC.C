...
std::string ParseBC::getOBCstr(const int id)
{
  ...
  else if( id == OBC_SLIDEWALL ) bc = "SlideWall";
  else                           bc = "";
  return bc;
}
...
{ 
  ...
  for (int odr=1; odr<=(int)NoBC; odr++) {
    ...
    if ( tp == SPEC_VEL ) {
      ...
      get_IBC_SpecVel(label_leaf, odr);//debug済み
...
/**
 @fn void ParseBC::setKeywordIBC(const char *keyword, const int m)
 @brief 内部境界条件の照合を行う
 @param keyword
 @param m エントリ番号
 @note SPEC_VEL_WHは陽には現れず，getXML_IBC_SpecVel()内で登録される
 */
void ParseBC::setKeywordIBC(const char *keyword, const int m)
{
...
/**
 @fn void ParseBC::setKeywordOBC(const char *keyword, const int m)
 @brief 外部境界条件のキーワードを照合し，コードを登録する
 @param keyword
 @param m エントリ番号
 */
void ParseBC::setKeywordOBC(const char *keyword, const int m)
{
  ...
  else if( !strcasecmp(keyword, "SlideWall"))           BaseBc[m].set_BCtype(OBC_SLIDEWALL);
  else {
