    ...
    for (int k=0; k<=kx+1; k++) {
      for (int j=0; j<=jx+1; j++) {
        for (int i=0; i<=ix+1; i++) {
          ...
          int s = bcp[m];
          int q = DECODE_CMP(d);
          Hostonly_ fprintf(fp, "[%4d %4d %4d], cmp[%2d], st=%1d: D(wesnbt) [%1d %1d %1d %1d %1d %1d] N [%1d %1d %1d %1d %1d %1d] NDAG [%1d %1d %1d %1d %1d %1d] DIAG=%1d cnv=%1d\n",
                            i, j, k, q, IS_FLUID(d),
                            (s>>BC_D_W)&0x1,
                            (s>>BC_D_E)&0x1,
                            (s>>BC_D_S)&0x1,
    ...
    for (int j=1; j<=jx; j++) {
      for (int i=1; i<=ix; i++) {
        ...
        if ( (ss == 0) && (TEST_BIT(s,ACTIVE_BIT)) )
        {
          ...
          Hostonly_ printf("\tError : Coefficient of diagonal element is zero at rank=%d (%d,%d,%d) : (wesnbt)[%1d %1d %1d %1d %1d %1d] : bid[%3d %3d %3d %3d %3d %3d]\n",
                           myRank, i,j,k,
...
unsigned long VoxInfo::replaceIsolatedFcell(int* bcd, const int fluid_id, const int* bid, const int m_NoCompo)
{
  int ix = size[0];
  int jx = size[1];
  int kx = size[2];
...
unsigned long VoxInfo::setBCIndexP(int* bcd,
                                   int* bcp,
                                   SetBC* BC,
                                   CompoList* cmp,
                                   int icls,
