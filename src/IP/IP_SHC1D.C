...
/**
 @fn void IP_SHC1D::setup(int* mid, Control* R, REAL_TYPE* G_org, const int Nmax, MediumList* mtl)
 @brief モデルIDのセットアップ
 @param mid ボクセル配列
 @param R Controlクラスのポインタ
 ...
 @param mtl
 */
void IP_SHC1D::setup(int* mid, Control* R, REAL_TYPE* G_org, const int Nmax, MediumList* mtl)
{
  unsigned i, j, k, m;
  int gd;
