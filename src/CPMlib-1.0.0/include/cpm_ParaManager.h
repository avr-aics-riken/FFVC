...
public:
   ...
   *  @param[in] pitch      ボクセルピッチ
   *  @param[in] obcid      全体空間の外部境界条件ID
   *  @param[in] maxVC      最大の袖数(袖通信用)
   *  @param[in] maxN       最大の成分数(袖通信用)
   *  @param[in] procGrpNo  領域分割を行うプロセスグループ番号
   *  @return 終了コード(CPM_SUCCESS=正常終了)
   */
  cpm_ErrorCode VoxelInit( int div[3], int vox[3], REAL_TYPE origin[3], REAL_TYPE pitch[3]
                         , int obcid[6], size_t maxVC=1, size_t maxN=3, int procGrpNo=0 );
   ...
   *  @return 終了コード(CPM_SUCCESS=正常終了)
   */
  cpm_ErrorCode VoxelInit( int vox[3], REAL_TYPE origin[3], REAL_TYPE pitch[3]
                         , int obcid[6], size_t maxVC=1, size_t maxN=3, int procGrpNo=0 );
...
////// 領域情報の取得関数 //////
  ...
  /** 自ランクのBCIDを取得
   *  @param[in] procGrpNo プロセスグループ番号(省略時=0)
   *  @return 自ランクのBCID整数配列のポインタ
   */
  const int* GetBCID( int procGrpNo=0 );
