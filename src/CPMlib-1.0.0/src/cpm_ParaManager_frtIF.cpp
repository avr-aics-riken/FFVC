...
#define _USE_S4D_
  ...
  #define cpm_GetBCID_               cpm_getbcid_
  ...
  #define cpm_GetBCID_               CPM_GETBCID
 ...
 *  @param[in]  obcid     全体空間の外部境界条件ID(サイズ6)
...
CPM_EXTERN
void
cpm_VoxelInit_( int *div, int *vox, REAL_TYPE *origin, REAL_TYPE *pitch, int *obcid
              , int *maxVC, int *maxN, int *procGrpNo, int *ierr )
{
  if( !div || !vox || !origin || !pitch || !obcid || !maxVC || !maxN ||
      !procGrpNo || !ierr )
  {
  ...
  // VoxelInit
  *ierr = paraMngr->VoxelInit( div, vox, origin, pitch, obcid
...
CPM_EXTERN
void
cpm_VoxelInit_nodiv_( int *vox, REAL_TYPE *origin, REAL_TYPE *pitch, int *obcid
                    , int *maxVC, int *maxN, int *procGrpNo, int *ierr )
{
  if( !vox || !origin || !pitch || !obcid || !maxVC || !maxN ||
      !procGrpNo || !ierr )
  {
    if( ierr ) *ierr = CPM_ERROR_INVALID_PTR;
    return;
  ...
  // VoxelInit
  *ierr = paraMngr->VoxelInit( vox, origin, pitch, obcid
                             , size_t(*maxVC), size_t(*maxN), *procGrpNo );
}
...
////////////////////////////////////////////////////////////////////////////////
/** 自ランクのBCIDを取得
 *  - GetBCIDのFortranインターフェイス関数
 *  @param[in]  procGrpNo プロセスグループ番号
 *  @param[out] bcid      自ランクのBCID(6wordの整数配列)
 *  @param[out] ierr      終了コード(0=正常終了、0以外=cpm_ErrorCodeの値)
 */
CPM_EXTERN
void
cpm_GetBCID_( int *bcid, int *procGrpNo, int *ierr )
{
  if( !procGrpNo || !bcid || !ierr )
  {
    if( ierr ) *ierr = CPM_ERROR_INVALID_PTR;
    return;
  }

 bcid[0] = bcid[1] = bcid[2] = bcid[3] = bcid[4] = bcid[5] = 0;
  ...
  const int *id = paraMngr->GetBCID( *procGrpNo );
  if( !id )
  {
    *ierr = CPM_ERROR_GET_BCID;
    return;
  }
