...
/**
 @fn void SklSolverCBC::NS_FS_E_CBC(void)
 @brief Fractional Step法でNavier-Stokes方程式を解く．バイナリ近似．
 */
void SklSolverCBC::NS_FS_E_CBC(void)
{
  ...
  TIMING__ PM.start(tm_frctnl_stp_sct);
  
  
  
  // >>> Fractional step sub-section 1
  TIMING__ PM.start(tm_frctnl_stp_sct_1); 
  ...
  // IN_OUT境界条件のときのフラグ処理
  if ( BC.has_InOut() ) {
    TIMING__ PM.start(tm_flip_bf);
    BC.flipDir_OBC(bcv, &C);
    TIMING__ PM.stop(tm_flip_bf, 0.0);
  }
  ...
  switch (C.AlgorithmF) {
    ...
    case Control::Flow_FS_AB2:
      ...
      TIMING__ PM.stop(tm_pseudo_vec, flop_count);

      TIMING__ PM.start(tm_pvec_flux);
      flop_count = 0.0;
      BC.mod_Pvec_Flux(vc, v0, bcv, tm, &C, v_mode, v00, flop_count);
      TIMING__ PM.stop(tm_pvec_flux, flop_count);
      break;
  ...
  switch (C.AlgorithmF) {
    case Control::Flow_FS_EE_EE:
      TIMING__ PM.start(tm_pvec_ee);
      flop_count = 0.0;
      cbc_ee_ (vc, sz, gc, &dt, v0, (int*)bcd, &flop_count);
      TIMING__ PM.stop(tm_pvec_ee, flop_count);
      break;
      
    case Control::Flow_FS_AB2:
      TIMING__ PM.start(tm_pvec_ab);
      flop_count = 0.0;
      ...
      TIMING__ PM.stop(tm_pvec_ab, flop_count);
      break;
      
    case Control::Flow_FS_AB_CN:
      TIMING__ PM.start(tm_pvec_abcn);
      flop_count = 0.0;
      ...
      TIMING__ PM.stop(tm_pvec_abcn, flop_count);
      
      TIMING__ PM.start(tm_pvec_abcn_df_ee);
      flop_count = 0.0;
      cbc_vis_ee_(vc, sz, gc, dh, &dt, v00, &rei, wv, v0, (int*)bcv, &half, &flop_count);
      TIMING__ PM.stop(tm_pvec_abcn_df_ee, flop_count);
      
      TIMING__ PM.start(tm_pvec_abcn_df_ee_BC);
      flop_count = 0.0;
      BC.mod_Vis_EE(vc, v0, half, bcv, tm, dt, v00, flop_count);
      TIMING__ PM.stop(tm_pvec_abcn_df_ee_BC, flop_count);
      break;
  ...
  if ( para_mng->IsParallel() ) {
    TIMING__ PM.start(tm_pvec_comm);
    if ( !dc_vc->CommBndCell(1) ) assert(0); // 1 layer communication
    TIMING__ PM.stop(tm_pvec_comm, comm_size*1.0*3.0); // ガイドセル数 x ベクトル
  }
  
  TIMING__ PM.stop(tm_frctnl_stp_sct_3, 0.0);
  // <<< Fractional step subsection 3
  
  
  
  // >>> Fractional step sub-section 4
  TIMING__ PM.start(tm_frctnl_stp_sct_4);
  ...
  TIMING__ PM.stop(tm_frctnl_stp_sct_4, 0.0);
  // <<< Fractional step subsection 4
  
  TIMING__ PM.stop(tm_frctnl_stp_sct, 0.0);
  // <<< Fractional step section
  ...
  TIMING__ PM.start(tm_poi_src_sct);
  ...
  TIMING__ PM.start(tm_div_pvec);
  flop_count = 0.0;
  cbc_div_(src0, sz, gc, &coef, vc, (int*)bcv, v00, &flop_count);
  TIMING__ PM.stop(tm_div_pvec, flop_count);
  
  // Poissonソース項の速度境界条件（VBC）面による修正
  TIMING__ PM.start(tm_poi_src_vbc);
  flop_count = 0.0;
  BC.mod_Psrc_VBC(src0, vc, v0, coef, bcv, tm, dt, &C, v00, flop_count);
  TIMING__ PM.stop(tm_poi_src_vbc, flop_count);
  ...
  if ( ICp->get_normType() == ItrCtl::p_res_l2_r) {
    TIMING__ PM.start(tm_poi_src_nrm);
    b2 = 0.0;
    cbc_div_cnst_(src0, sz, gc, &b2, (int*)bcp, &flop_count);
    b2 = sqrt(b2);
    TIMING__ PM.stop(tm_poi_src_nrm, flop_count);
    
    if ( para_mng->IsParallel() ) {
      TIMING__ PM.start(tm_poi_src_comm);
      SKL_REAL m_tmp = b2;
      para_mng->Allreduce(&m_tmp, &b2, 1, SKL_ARRAY_DTYPE_REAL, SKL_SUM, pn.procGrp);
      TIMING__ PM.stop(tm_poi_src_comm, 2.0*np_f*(SKL_REAL)sizeof(SKL_REAL) ); // 双方向 x ノード数
    }
  }
  
  TIMING__ PM.stop(tm_poi_src_sct, 0.0);
  // <<< Poisson Source section
  ...
  TIMING__ PM.start(tm_poi_itr_sct);
  
  if ( C.Mode.Log_Itr == ON ) {
    TIMING__ PM.start(tm_hstry_itr);
    Hostonly_ H->printHistoryItrTitle(fp_i);
    TIMING__ PM.stop(tm_hstry_itr, 0.0);
  }
  ...
  for (ICp->LoopCount=0; ICp->LoopCount< ICp->get_ItrMax(); ICp->LoopCount++) {
    ...
    TIMING__ PM.start(tm_poi_itr_sct_1);
    ...
    if ( C.isForcing() == ON ) {
      TIMING__ PM.start(tm_force_src);
      flop_count=0.0;
      BC.mod_Psrc_Forcing(src1, v, bcd, &C, v00, flop_count);
      TIMING__ PM.stop(tm_force_src, flop_count);
    }
    
    // 内部周期境界部分のディリクレソース項
    //TIMING__ PM.start(tm_prdc_src);
    //BC.InnerPrdc_Src(dc_wk2, dc_p, dc_bcd);
    //TIMING__ PM.stop(tm_prdc_src, flop_count);
    
    TIMING__ PM.stop(tm_poi_itr_sct_1, 0.0);
    // <<< Poisson Iteration subsection 1
    ...
    TIMING__ PM.start(tm_poi_itr_sct_4);
    
    // 速度のスカラポテンシャルによる射影と速度の発散の計算 src1はdiv(u)のテンポラリ保持に利用
    TIMING__ PM.start(tm_prj_vec);
    flop_count = 0.0;
    cbc_update_vec_(v, src1, sz, gc, &dt, dh, vc, p, (int*)bcp, (int*)bcv, v00, &coef, &flop_count);
    TIMING__ PM.stop(tm_prj_vec, flop_count);
    
    // セルフェイス速度の境界条件による修正
    TIMING__ PM.start(tm_prj_vec_bc);
    flop_count=0.0;
    BC.mod_div(src1, bcv, coef, tm, v00, flop_count);
    TIMING__ PM.stop(tm_prj_vec_bc, flop_count);
    
    // Forcingコンポーネントによるセルセンター速度の修正
    if ( C.isForcing() == ON ) {
      TIMING__ PM.start(tm_prj_frc_mod_cc);
      flop_count=0.0;
      BC.mod_Vcc_Forcing(v, bcd, &C, dt, v00, flop_count);
      TIMING__ PM.stop(tm_prj_frc_mod_cc, flop_count);
    }

    // 周期型の速度境界条件
    TIMING__ PM.start(tm_vec_BC);
    flop_count=0.0;
    BC.OuterVBC_Periodic(dc_v);
    BC.InnerVBC_Periodic(dc_v, dc_bcd);
    TIMING__ PM.stop(tm_vec_BC, flop_count);
    
    TIMING__ PM.stop(tm_poi_itr_sct_4, 0.0);
    // <<< Poisson Iteration subsection 4
  ...
  TIMING__ PM.stop(tm_poi_itr_sct, 0.0);
  // <<< Poisson Iteration section
  
  
  
  /// >>> NS Loop post section
  TIMING__ PM.start(tm_NS_loop_post_sct);
  
  // 同期
  if ( para_mng->IsParallel() ) {
    TIMING__ PM.start(tm_vectors_comm);
    dc_v->CommBndCell(guide);
    TIMING__ PM.stop(tm_vectors_comm, 2*comm_size*(SKL_REAL)guide*3.0);
  }
  
  // 外部領域境界面での速度や流量を計算 > 外部流出境界条件の移流速度に利用
  TIMING__ PM.start(tm_domain_monitor);
  flop_count=0.0;
  DomainMonitor( BC.get_OBC_Ptr(), &C, flop_count );
  TIMING__ PM.stop(tm_domain_monitor, flop_count);
  
  // 流出境界のガイドセル値の更新と速度境界条件
  TIMING__ PM.start(tm_VBC_update);
  flop_count=0.0;
  BC.InnerVBC(v, bcv, tm, v00, flop_count);
  BC.OuterVBC(v, vc, bcv, tm, dt, &C, v00, flop_count);
  TIMING__ PM.stop(tm_VBC_update, flop_count);
  
  // 非同期にして隠す
  if (C.LES.Calc==ON) {
    TIMING__ PM.start(tm_LES_eddy);
    flop_count = 0.0;
    cbc_eddy_viscosity_(vt, sz, gc, dh, &C.Reynolds, &C.LES.Cs, v, (int*)bcv, range_Ut, range_Yp, v00);
    TIMING__ PM.stop(tm_LES_eddy, flop_count);
    
    if ( para_mng->IsParallel() ) {
      TIMING__ PM.start(tm_LES_eddy_comm);
      if( !dc_vt->CommBndCell(guide) ) assert(0);
      TIMING__ PM.stop(tm_LES_eddy_comm, comm_size*(SKL_REAL)guide);
    }
  }
  ...
  TIMING__ PM.stop(tm_NS_loop_post_sct, 0.0);
  // >>> NS loop post section
}