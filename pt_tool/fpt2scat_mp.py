...
######################################
def usage():
	print('\t$ fpt2scat_mp.py -a   // specify ascii mode (if not, binary)')
	print('\t$                -n <number of processes>')
	#print('\t$                -p <number of emitting points to be processed>')
	print('\t$                -s <number of steps to be processed>')
	sys.exit(0)
...
def rw_particle(np, s, r, n, tp, rp):
	...
	for q in range(np):
		...
		if mode == 'ascii':
			...
			tm = readChunkBinary(stp, rnk, lst)
...
def readChunkBinary(stp, rank, lst):
...
def readBinaryHeader(step, rank):
...
def main():
	...
	# スクリプト名を除いたコマンドライン引数
	argv = sys.argv[1:]
	opts, args = getopt.getopt(argv, "an:p:h")
...
# >>>>>>>>>>>>>>>>>>>>>
	...
	npt_files.sort()
	#print('\n'.join(npt_files))
	print('\tNumber of files = %d' % len(npt_files))
	...
	print('\tNumber of steps included in current directory = %d' % num_step)
	if nStep == 0:
		nStep = num_step
	print('\tNumber of steps to process = %d' % nStep)
	...
	if mode == 'ascii':
		...
		for j in range(len(stepList)):
			stp = stepList[j]
			rank= rankList[j]
			num_p = readBinaryHeader(stp, rank)
			nparList.append(num_p)
	
	total_pt = sum(nparList)
	print('\tTotal particles = %d' % total_pt)


	# 並列処理時の担当範囲を決める
	sz = []
	st = []
	div = int(nStep/num_proc)
	if nStep % div != 0:
		div += 1
	...
	# 並列処理
	jobs = []
	ratio = Value('I', total_pt) # unsigned 桁落ちさせないため
